...
class OptionManager
{
	 ...
	 *
	 * Seçenek yoksa veya bir değeri yoksa, dönüş değeri `false` olacaktır. Bu, bir
	...
	public function get(string $key, $default = null)
	{
		...
		if (!app_installing()) {
			// Var olmayan seçeneklerin birden çok sorguyu tetiklemesini önle
	 ...
	 * @return void
	 */
	public function protectOption(string $key): void
	{
		if ($key === 'alloptions' || $key === 'notoptions') {
			die("`$key` adlı seçenek, Candtry Platform'un korumalı seçeneğidir ve düzenlenemez.");
		}
	}

	/**
	 * Seçeneğin niteliğine bağlı olarak çeşitli seçenek değerlerini temizler.
	 *
	 * @since 1.0
	 *
	 * @param string $option   Seçenek adı.
	 * @param mixed  $value    Temizlenmemiş değer.
	 *
	...
	/**
	 * Varsa, tüm otomatik yüklenen seçenekleri veya tüm seçenekleri yükler ve önbelleğe alır.
	 *
	 * @since 1.0
	 *
